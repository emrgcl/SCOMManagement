...
Write-Verbose "Deleting node $($Node.ID)"
}
} 
 
...
 
[xml]$mpxml = Get-Content -Path $MPXmlPath -Encoding Unicode
 
...
        {
            
            $UniqueReferredOverride |Remove-CurrentNode
 
        }
 
}
...
 
Try {
# SCOM exports xml with UTF16-LE we need to set the encoding correctly.
$UTf16le = [System.Text.Encoding]::Unicode
$StreamWriter = New-Object -TypeName System.IO.StreamWriter($MPXmlPath,$false,$UTf16le)
 
$mpxml.Save($StreamWriter )
Write-Verbose "Succesfully saved to '$MPXmlPath'"
} Catch 
 
{
 
Throw "Error occured during save operation exiting script. Error: $($_.Exception.Message)"
 
}
Finally {
 
